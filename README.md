...
* Use [uv](https://docs.astral.sh/uv/) to manage the development environment and to test against multiple versions of Python
* Makefile wrapping around `uv` for convenience on Unix. Run `make help` inside the generated project for details.
...
* Optional use of [MyST](https://myst-parser.readthedocs.io/) Markdown for the documentation sources and docstrings
* Automatically generated API documentation (via [sphinx-autodoc2](https://sphinx-autodoc2.readthedocs.io/) for the Markdown docs, or `sphinx.ext.autodoc`/`autosummary` for the reStructuredText docs)
* Versions menu in online documentation with [Docs Versions Menu][]
...
* `markdown_docs`: Whether the documentation sources are written in MyST Markdown (`y`) or reStructuredText (`n`). Only relevant if `sphinx_docs` is enabled.
* `markdown_docstrings`: Whether docstrings are parsed as MyST Markdown (`y`) or as reStructuredText field lists (`n`). Only relevant if `markdown_docs` is enabled.
* `support_py310`: Does the package support Python 3.10?
* `support_py311`: Does the package support Python 3.11?
* `support_py312`: Does the package support Python 3.12?
* `support_py313`: Does the package support Python 3.13?
* `support_py314`: Does the package support Python 3.14?
...
*   Declare runtime dependencies in the `[project]` `dependencies` of `pyproject.toml`, and development dependencies in the `[dependency-groups]` section. There are no additional [pip requirement files](https://pip.pypa.io/en/stable/user_guide/#requirements-files) (These are for app deployment, not for packages!).
...
*   Review the classifiers in `pyproject.toml`. The full list of PyPI classifiers can be found [here](https://pypi.python.org/pypi?:action=list_classifiers).
